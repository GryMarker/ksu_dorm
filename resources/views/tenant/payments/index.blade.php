...
<x-ksu-layout page-title="Student Payments">
    <div class="space-y-8">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-ksu-900 sm:text-3xl">Student Payment Records</h1>
                <p class="mt-1 text-sm text-slate-600">
                    Submit dorm payment records with your own amount for Dorm Master review.
                </p>
            </div>
            <x-ksu-button :href="route('tenant.dashboard')" size="sm" variant="outline">
                Back to Dashboard
            </x-ksu-button>
        </div>
            ...
            <form method="POST" action="{{ route('tenant.payments.store') }}" class="space-y-6">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div class="space-y-2 md:col-span-2">
                        <x-input-label for="student_note" value="Student Note (optional)" />
                        <textarea
                            id="student_note"
                            name="student_note"
                            rows="3"
                        ...
                        <x-input-error :messages="$errors->get('student_note')" />
                    </div>
                </div>
                                ...
                                <tr>
                                    ...
                                    <td class="px-4 py-3">
                                        <x-ksu-badge :variant="match ($payment->status) {
                                            StudentPayment::STATUS_APPROVED => 'approved',
                                            StudentPayment::STATUS_REJECTED => 'rejected',
                                            default => 'pending',
                                        }">
                                    ...
                                    <td class="px-4 py-3 text-xs text-slate-500">
                                        @if ($payment->review_note)
                                            {{ $payment->review_note }}
                                        @elseif ($payment->student_note)
                                            <span class="text-slate-600">Student note: {{ $payment->student_note }}</span>
                                        @else
                                            &mdash;
                                        @endif
                                    </td>
